...
<script>
  ...
  import { getTheUserId} from '../../utils/auth'
  ...
  export default {
    ...
    created() {
      ...
      storage.get('userName', function (error, data) {
        that.userName = data.value
      })
      storage.get('userId', function(error, data) {
        // alert(data.value)
        // return(data.value)
        that.userId = data.value
        that.getList()
      })
      // this.userId = getTheUserId('userId')
      // 7this.userId = getStorage('userId')
