...
// import { getCookies, setCookies } from '../../../../dist/electron/main'
// import { getCookies, setCookies, testPath, setStorage, getStorage } from '@/utils/cookies'
import { testPath, setStorage, getStorage } from '@/utils/cookies'
/* eslint-disable */
export default {
  ...
  created() {
    ...
    testPath()
      ...
      storage.set('footbal', { value: 'foo' }, function(error) {
          if (error) throw error;
      })
      storage.get('footbal', function(error, data) {
          alert(data.value)
      })*/
    if(getStorage('ip')) {
      this.loginForm.ipConfig = getStorage('ip')
    }
    if(getStorage('port')) {
      this.loginForm.port = getStorage('port')
    }
    if(getStorage('username')) {
      this.loginForm.username = getStorage('username')
    }
    setStorage('ip', this.loginForm.ipConfig)
    setStorage('port', this.loginForm.port)
  },
  destroyed() {
  }
}
